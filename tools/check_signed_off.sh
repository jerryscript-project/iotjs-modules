...
function print_help
{
 ...
 echo "'$PREFIX-DCO-1.0-Signed-off-by: NAME EMAIL', where NAME and EMAIL must"
...
then
 ...
 required_signed_off_by_line="$PREFIX-DCO-1.0-Signed-off-by: $author_name $author_email"
 ...
 then
  ...
  if echo "$actual_signed_off_by_line" | grep -q -v '^$PREFIX-DCO-1.0-Signed-off-by:'
  then
   echo -e "\e[1;33mSigned-off-by message is incorrect. The following line should be at the end of the $commit_hash commit's message: '$required_signed_off_by_line'. \e[0m"
   exit 1
  fi
